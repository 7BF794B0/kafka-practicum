...
class ClientAPI:
  def __init__(self, kafka_configs: Iterable[Dict], es_config):
    self.producers = [Producer(cfg) for cfg in kafka_configs]
    primary_config = kafka_configs[0]
  ...
  def search_product(self, query):
    ...
    for producer in self.producers:
      producer.produce('client_requests', value=payload)
      producer.flush()
...
if __name__ == '__main__':
  ...
  api = ClientAPI(kafka_configs, es_config={'host': 'elasticsearch', 'port': 9200})
